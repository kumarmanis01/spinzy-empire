...
const nodeArgs = ['-r', 'ts-node/register/transpile-only', 'scripts/runAlertEvaluator.ts'];
...
p.on('close', (code) => {
  ...
  // Ensure the dry-run logged at least a startup event
  if (!out.includes('evaluator_starting') && !out.includes('alert_dry_run')) {
